# onlastik_arkalastik_ayir metodu ile dosya yolundaki lastikleri ön lastik  arka lastik olarak iki klasörde ayırmamızı sağlar.Aynı zamanda lastiğin tamamının olmadığı resimleride siler
# lastik tespiti,lastiğin yanak kısmından düz şerit elde edilmesi işlemleri yapılır. Easyocr çalıştırılarak sonuç almamızı sağlar.
...
def main():
    global path
    ls.onlastik_arkalastik_ayir(path)  
    ...
    for f in filenames:
        subfolderpath = os.path.join(mainpath, f)
